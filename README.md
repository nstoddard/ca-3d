...
To experiment with different CA rules, modify the `update()` method in `src/ca.rs`. A couple of interesting rules are: B6,7/S5,7,8 (has a diagonal spaceship, but quickly expands to fill the whole space), B6,9/S5,6,7,10 (has a similar spaceship, but it seems somewhat more rare), B5/S4,5,10.
